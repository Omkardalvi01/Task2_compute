...
def main():
    ...
    num_of_doors = st.selectbox('Number of doors', options=[1,2,4],placeholder="Choose an option")
    body_style = st.selectbox('Body Style',options=['convertible', 'hatchback', 'sedan', 'wagon', 'hardtop'],placeholder="Choose an option")
    asp = st.selectbox('Aspiration', options=['std','turbo'],placeholder="Choose an option")
    l = st.number_input('Length',value=None, min_value=0.0)
    w = st.number_input('Width',value=None, min_value=0.0)
    h = st.number_input('Height', value=None, min_value=0.0)
    weight = st.number_input('Curb Weight',value=None, min_value=0.0)
    fuel_type = st.selectbox('Fuel-Type',options=['gas','diesel'],placeholder="Choose an option")
    fuel_sys = st.selectbox('Fuel-System',options=['mpfi', '2bbl', 'mfi', '1bbl', 'spfi', '4bbl', 'idi', 'spdi'],placeholder="Choose an option")
    engine_loc = st.selectbox('Engine Location', options=['front', 'rear'],placeholder="Choose an option")
    drive_wheels = st.selectbox('Drive Wheels',options=['rwd', 'fwd', '4wd'],placeholder="Choose an option")
    hp = st.number_input('Horsepower',value=None, min_value=0.0)
    p_rpm = st.number_input('Peak RPM',value=None, min_value=0.0)
    engine_type = st.selectbox('Engine Type',options=['dohc', 'ohcv', 'ohc', 'l', 'rotor', 'ohcf', 'dohcv'],placeholder="Choose an option")
    num_of_cyliner = st.selectbox('num_of_clinders', options=[2,3,4,5,6,8,12],placeholder="Choose an option")
    bore = st.number_input('Bore',value=None, min_value=0.0)
    stroke = st.number_input('Stroke',value=None, min_value=0.0)
    compression_ratio =st.number_input('Compression Ratio',value=None, min_value=0.0)
